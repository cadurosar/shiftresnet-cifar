...
class ResNet(nn.Module):
    def __init__(self, block, num_blocks, reduction=1, num_classes=10):
        ...
        self.in_planes = int(88 / self.reduction)

        self.conv1 = nn.Conv2d(3, self.in_planes, kernel_size=3, stride=1, padding=1, bias=False)
        self.bn1 = nn.BatchNorm2d(self.in_planes)
        self.layer1 = self._make_layer(block, self.in_planes, num_blocks[0], stride=1)
        self.layer2 = self._make_layer(block, int(88*2 / self.reduction), num_blocks[1], stride=2)
        self.layer3 = self._make_layer(block, int(88*4 / self.reduction), num_blocks[2], stride=2)
        self.linear = nn.Linear(int(88*4 / self.reduction), num_classes)
